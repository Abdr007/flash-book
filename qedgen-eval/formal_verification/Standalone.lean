/-
Standalone machine-check of the QEDGen-generated haircut dust-conservation
obligation, decoupled from the qedsvm support library (whose native sBPF-VM
plugin fails to build on this platform — unrelated to the proof).

The definitions + theorem statement are copied VERBATIM from the QEDGen-generated
`Spec.lean` (only `Pubkey` is stubbed, since the bound never uses it). This is the
exact obligation CBMC/Kani could not discharge with the real 1e9 divisor.
-/
import Mathlib.Tactic

namespace HaircutStandalone
