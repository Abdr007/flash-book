...
/// The subset of a Pyth price clober consumes.
...
pub fn get_price_no_older_than_full(
    ...
    require!(data.len() >= 8, CloberError::OutOfRange);
    require!(
    ...
    require!(data.len() >= 41, CloberError::OutOfRange);
    ...
    require!(data[40] == 1, CloberError::OracleTooStale);
    ...
    require!(&data[m..m + 32] == feed_id, CloberError::OracleTooStale); // MismatchedFeedId
    let price = i64::from_le_bytes(
        data[m + 32..m + 40]
            .try_into()
            .map_err(|_| error!(crate::CloberError::OutOfRange))?,
    );
