...
pub fn require_seq_encodable(seq: u64) -> Result<()> {
    require!(
        ...
        crate::errors::CloberError::OrderSeqExhausted
    );
    Ok(())
}
...
/// in raw account dumps. "FB BK MK BK 01 …" = Clober / Book / Market.
...
impl<'a> MarketBookHandle<'a> {
    ...
    pub fn from_account_data(data: &'a mut [u8]) -> Result<Self> {
        ...
        require!(
            ...
            crate::errors::CloberError::WrongTrader,
        );
    ...
    pub fn validate_node_links(account_data: &[u8]) -> Result<()> {
        ...
        for (best, root) in [
            ...
            if root == NIL {
                require!(best == NIL, crate::errors::CloberError::OutOfRange);
            } else {
        ...
        // sits at byte offset 0 (same slot the DFS read as `left`).
        {
            ...
            while free != NIL {
                ...
                let ord = off / NODE_TOTAL_BYTES;
                require!(!visited[ord], crate::errors::CloberError::OutOfRange);
                visited[ord] = true;
    ...
    pub fn alloc_node(&mut self) -> Result<DataIndex> {
        ...
        require!(
            ...
            crate::errors::CloberError::BufferFull
        );
    ...
    pub fn decrement_size_at(&mut self, idx: DataIndex, delta: u64) -> Result<u64> {
        ...
        let new_size = node
            ...
            .size_lots
            .checked_sub(delta)
            .ok_or_else(|| error!(crate::errors::CloberError::ArithmeticOverflow))?;
