...
/// All Clober error codes. Numbered with stable ranges so downstream
...
pub enum CloberError {
    ...
    #[msg("Resting order price too far from the oracle — anti-stuffing band")]
...
impl<T> OrOverflow<T> for Option<T> {
    fn or_overflow(self) -> Result<T> {
        self.ok_or_else(|| error!(CloberError::ArithmeticOverflow))
    }
    fn or_underflow(self) -> Result<T> {
        self.ok_or_else(|| error!(CloberError::ArithmeticUnderflow))
    }
    fn or_div_zero(self) -> Result<T> {
        self.ok_or_else(|| error!(CloberError::DivisionByZero))
    }
}
