...
pub fn cpi_delegate(
    ...
    require_keys_eq!(
        *accounts.delegation_program.key,
        DELEGATION_PROGRAM_ID,
        crate::CloberError::Unauthorized
    );
    ...
    // 2. Stage the account's data into the buffer.
    {
        ...
        require!(dst.len() == src.len(), crate::CloberError::OutOfRange);
    ...
    **accounts.payer.try_borrow_mut_lamports()? = accounts
        ...
        .ok_or(crate::CloberError::ArithmeticOverflow)?;
...
pub fn process_external_undelegate<'info>(
    ...
    require!(buffer.is_signer, crate::CloberError::Unauthorized);
    require_keys_eq!(
        *buffer.owner,
        DELEGATION_PROGRAM_ID,
        crate::CloberError::Unauthorized
    ...
    require_keys_eq!(*delegated.key, derived, crate::CloberError::WrongMarket);
