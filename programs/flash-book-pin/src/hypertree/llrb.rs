#[cfg(test)]
use super::red_black_tree::RedBlackTreeTestHelpers;
use crate::trace;
use super::{
    get_helper, get_mut_helper, Color, DataIndex, GetRedBlackTreeData,
...
pub struct LLRB<'a, V: Payload> {
    ...
    phantom: core::marker::PhantomData<&'a V>,
}
...
impl<'a, V: Payload> LLRB<'a, V> {
    ...
    fn remove_by_value(&mut self, value: &V) {
        use super::HyperTreeReadOperations;
...
// Internal RBT tests are vendored from upstream Manifest. They use crate-root
// import paths that don't translate to our module layout. Disable in our
// crate — Manifest validates them upstream + Certora formally verifies the
// RBT invariants.
#[cfg(all(test, feature = "hypertree-internal-tests"))]
mod test {
    use super::super::{
