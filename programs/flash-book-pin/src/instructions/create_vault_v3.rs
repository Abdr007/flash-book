//! create_vault_v3 — a strategist creates a new managed vault account, PDA
//! `[b"vault_v3", strategist, vault_id]`. Pure state init (no funds move); the
//! vault accepts deposits once a TraderState is bootstrapped for its PDA.
//! Faithful port of the Anchor `create_vault_v3`.
//!
//! accounts: [strategist (signer, payer, w), vault (PDA, w, uninit),
//!            system_program]
//! data: [vault_id u8][perf_fee_bps u32][name [u8; 32]]   — 37 bytes
...
/// Max performance fee — 50% (mirrors anchor's `perf_fee_bps <= 5_000`).
...
pub fn process(program_id: &Pubkey, accounts: &[AccountInfo], data: &[u8]) -> ProgramResult {
    ...
    let vault_id = data[0];
    let perf_fee_bps = u32::from_le_bytes(data[1..5].try_into().unwrap());
    let mut name = [0u8; 32];
    name.copy_from_slice(&data[5..37]);
    ...
    // Create the PDA (unique per (strategist, vault_id)).
