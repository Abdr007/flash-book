...
| O-1 | LOW | ER | **DOCUMENTED (design decision)** | `from_account_data` bounds-checks the 6 header indices but not internal RBT `left/right/parent` links (hot path uses unchecked `get_helper`). A malicious-ER-committed book with an OOB child pointer → L1 traversal panic (book DoS; requires a rogue ER validator, fills still §3.2-protected). **Not rushed — the fix is a real perf/refactor tradeoff:** (a) a full slab-link walk in `from_account_data` is O(capacity) on the *hot path* (fine at 256 nodes, too costly at 10k); (b) switching the traversal accessors to `get_helper_checked` is O(log n)/op but a refactor of the *proven* RBT. **Recommendation: (b)** — fail-closed `OutOfRange` revert instead of a panic — as its own focused, re-proven change. |
| O-2 | LOW | oracle | **FIXED** (`3fbef44`) | Pyth/Lazer permissionless paths didn't future-reject `publish_time` (the authority path did) — a future-dated, trust-anchor-signed timestamp saturated staleness to 0. Added `require!(published <= now)` to both (`pyth_oracle.rs`, `update_oracle_from_lazer`). |
...
| H-1..H-4 | INFO | hygiene | **PARTLY DONE** (`3fbef44`) | Removed: **8 dead fns** (`lot.rs` `is_zero`×2/`ticks_delta`, `order.rs` `is_taker`/`fifo_key`, `state.rs` `side_enum`, `haircut.rs` `usd_to_quote_lots`, `er.rs` `delegation_metadata_pda`) + **2 dead constants** (`USD_DECIMALS`, `LOT_EPSILON`) + stale comments + the cascade unused import, build-verified. **Remaining for a focused cleanup PR** (verified-dead, deferred to avoid a cascade/IDL-sync error this pass): `er.rs::cpi_undelegate` (+ its now-dead `UndelegateAccounts`/discriminator cascade), `state_v2` `insert_seat`/`lookup_min_index`, the 23 unused error variants (need IDL sync). The 21 *unwired matcher feature-modules* are intentional proven future features — **not** deleted; recommend an `unwired/` namespace. |
