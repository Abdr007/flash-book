...
//   3. after FLP_REFRESH_MIN_SLOTS (~10 slots) elapse -> re-quote ALLOWED
import fs from "fs";
import os from "os";
...
await sendAs(signer, await program.methods.initializeMarket(ref.params, new BN(1000)).accountsPartial({ authority: signer.publicKey, baseMint: base.publicKey, quoteMint: QUOTE, baseVault: OBV, quoteVault: VAULT, oracleAccount: OOR, market: M, insuranceFund: INS, flpExposure: FLP, systemProgram: sys }).instruction());
...
const refreshIx = (who) => program.methods.flpRefreshQuotes().accountsPartial({ authority: who, market: M, marketBook: BOOK, flpExposure: FLP }).instruction();
...
console.log("3) wait ~24s (> FLP_REFRESH_MIN_SLOTS=50 ≈ 20s) then re-quote → ALLOWED");
await sleep(24000);
let sig3 = null;
