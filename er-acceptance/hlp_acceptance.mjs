...
//   2. FLP posts a resting maker ASK (flp_post_maker_order) — verifies the BPF
...
//      commitment with maker = the flp_exposure PDA
//   4. assert the ring recorded exactly one produced commitment -> the FLP order
//      rested, was crossed, and is ring-committed (ready for permissionless
//      apply_flp_fill, which is CI-verified end-to-end).
...
console.log("1) FLP posts a resting maker ASK 1 @ 100_000 (flp_post_maker_order)");
const postSig = await send(await program.methods.flpPostMakerOrder(1, new BN(1), new BN(100000), new BN(0)).accountsPartial({ authority: signer.publicKey, market: M, marketBook: BOOK, flpExposure: FLP }).instruction());
...
ok(produced === 1, `matcher pushed exactly one commitment for the crossed FLP quote (produced=${produced})`);
ok(settled === 0, `commitment pending settlement (settled=${settled}) — ready for permissionless apply_flp_fill`);
