...
//   3. a taker crosses the pool's best ask -> ring records an LP-maker fill
//   4. lp_refresh_quotes again -> cancels stale + reposts (idempotent)
...
console.log("1) pool AUTO-QUOTES (lp_refresh_quotes) — deterministic ladder from oracle + inventory");
const sig1 = await send(await program.methods.lpRefreshQuotes().accountsPartial({ authority: signer.publicKey, market: M, marketBook: BOOK, lpExposure: LP }).instruction());
...
console.log("3) re-quote (lp_refresh_quotes again) — cancel stale + repost");
// Quotes still resting from stage 1 are rate-limited for LP_REFRESH_MIN_SLOTS
