...
| `matcher/liquidation` | 6 | Health price is always one of the two real sources and the worse one for the side (long and short), fresh mark equals worse-of, stale mark falls back to oracle-only, and the mark-manipulation property (`manipulated_mark_yields_no_usable_equity`): a mark manipulated in the attacker's favour can never move the health price past the honest live oracle, yielding no additional usable equity. |
