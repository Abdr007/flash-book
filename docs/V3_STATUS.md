...
| 21   | Modular wrapper programs | **Phase 1 shipped** — 3 sister Anchor programs (`flash-book-orders`, `flash-book-flp`, `flash-book-vaults`) with their own program IDs and `ping`-only `lib.rs`. All 4 programs build clean, IDLs generated, .so files in `target/deploy/`. Phases 2-4 (CPI surface, state migration, sunset) per [`V3_WAVE21_MODULAR.md`](./V3_WAVE21_MODULAR.md) — 3-4 weeks. |
