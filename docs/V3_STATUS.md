...
| 21   | Modular wrapper programs | **Phases 1+2+3a shipped.** Phase 1: 3 sister programs deployable (orders/flp/vaults). Phase 2: core's `place_limit_order_v2_cpi` + wrapper authority PDAs proven end-to-end. Phase 3a: TriggerOrderAccountV3 fully migrated to flash-book-orders (place + execute via CPI + cancel + SDK). Remaining 3b-3d (TWAP/iceberg/bracket/FLP/vault account migrations) follow the same template. Per [`V3_WAVE21_MODULAR.md`](./V3_WAVE21_MODULAR.md). |
