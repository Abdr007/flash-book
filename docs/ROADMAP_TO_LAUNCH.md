...
|---|---|---|---|---|
| 1.1 | Certora solvency spec — compile the harness, wire real handler externs, run the prover, add to CI (closes G1) | eng | **RUNNING — PARTIAL (1 slice green, G1 not yet closed).** Env fully unblocked: `certora-cli 8.17.1` + `cargo-certora-sbf 0.3.5` + public `cvlr` 0.6.1 / `cvlr-solana` 0.5.0 (git, no private registry) + license live. Harness rebuilt from the EVM-CVL scaffold to the REAL Solana `#[rule]` pattern (the old `.spec`/`method f` form does not drive `process:sbf`). **First real cloud pass: `solvency_preserved_simple_withdraw` — VERIFIED, non-vacuous (`rule_not_vacuous_cvlr` green), exit 0** — solvency preserved against the REAL `assess_solvency_full` invariant over all `u64`, on `flash_book.so` built `--features certora` with `v1.53-certora` tools. CI job runs `certoraSolanaProver` (secret-gated). **RESIDUAL to close G1:** (a) the real gate symbol `check_simple_withdraw` hits a Prover pointer-analysis limit (Anchor `require!` error path memcpy's an unclassified `solana-address` global) → needs a faithful summary; (b) 18 more balance-mutating handlers; (c) symbolic Anchor `Context` construction for the full per-handler dispatch | 1 rule VERIFIED non-vacuous in CI; 18 handlers + gate-symbol summary remain |
...
- **1.1** Certora solvency spec — **WIRED, VENDOR-WAIT** (`certora/`): parametric `solvencyPreserved(method f)` + `surplusNeverInvented` specs + conf + build.sh + cvt_summaries + cvt_inlining + CI job (gates when `CERTORAKEY` set, skips otherwise). BLOCKER: needs the Certora Solana license + `cvlr` SDK + cloud run — NOT yet `VERIFICATION SUCCESSFUL`. Arithmetic is Kani-proven today; Certora adds all-paths reachability.
