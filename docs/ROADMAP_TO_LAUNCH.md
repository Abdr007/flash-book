...
| 2.3 | on-chain payout walk for referrer/builder/creator shares (today emit-only) | eng | **DESIGNED — ready to build (escrow-accrual, trustless).** Today the three shares are computed from `net_fee` but only `emit!`-ed (`ReferralOwedEvent`/`BuilderFeeOwedEvent`/`CreatorFeeOwedEvent`, lib.rs ~5615/5640/5660); the backing tokens sit in the quote vault as unattributed protocol surplus (`total_fees_collected`, never reserved), so payout relies on an off-chain integrator's honesty. **Design** — make it trustless without bloating the ER hot path: (1) new `FeeAccrual` PDA `seeds=[b"fee_accrual", recipient]` holding `accrued_quote_lots: u64` + `recipient` + bump; (2) in `apply_fill`, the accrual account is **optional** in `remaining_accounts` — if present for a recipient, `min(share, remaining_surplus)` is deducted from the protocol surplus and added to `accrued_quote_lots` (hot path touches ≤3 extra accts, only when opted in; **absent ⇒ exact current emit-only behaviour**, zero regression); (3) new cold `claim_fee_accrual` ix (permissionless, recipient-signed) transfers the accrued balance from the quote vault to the recipient ATA and zeroes the accrual, gated by the existing `verify_protocol_solvency`/Residual≥0 floor. **Invariants**: enforce `fee_contribution_bps + referrer + builder + creator shares ≤ BPS_DENOM` (or surplus-cap each accrual, mirroring the maker-rebate `min(taker_fee_paid)` cap) so total distributed never exceeds `net_fee` collected. **Conservation**: extends the proven identity to `V = C_tot + I + FeeAccrued + Residual` — accrual moves surplus→FeeAccrued (Residual-neutral), claim moves vault→recipient against FeeAccrued; needs a `ResidualConservation.lean` companion + a runtime sequence-fuzz row. Needs a devnet cycle. |
...
- **Program changes** (need a devnet deploy + live-re-verify cycle each): 2.3,
  4.5 (2.2 #315; 4.1 #316; 3.3 disp.; 4.3 #318; **4.4 wired** — OI-crowding surcharge, default-off migration). (**4.8 CLOSED** + devnet-accepted, PR #300; the H-B cancel-lock
