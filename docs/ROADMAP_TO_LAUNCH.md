...
| Item | Scope | Owner | Status | Evidence |
|---|---|---|---|---|
| 3.1 | per-source-domain realizable credit: cap each profitable leg's usable PnL by `credit_rate = min(1, backing/claims)` of the opposing side of that same market — manipulated/thin/stale market ⇒ credit collapses ⇒ paper profit can't back margin/cure/withdraw (closes the HL-$20M oracle-pump class) | eng | **(a) LEAN-PROVEN ✅** — `formal_verification/lean/PerDomainCredit.lean` (compiles clean, no sorry): `usable ≤ pnl`, `zero-rate ⇒ zero credit`, `no-backing ⇒ zero usable credit` (the marquee), `creditRate ≤ BPS`, at unbounded width. **(b) engine-wiring + (c) deploy+live-verify = REMAINDER** (multi-week, settlement-adjacent). Kani local check also SUCCESSFUL (not CI-resident, ~300-700s) | (a) done; (b)(c) live devnet |
