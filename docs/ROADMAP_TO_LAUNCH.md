...
- **1.4** proven-wrapper enforcement — `programs/flash-book/src/proven_wrapper_enforcement.rs` (2 CI tests). Closes G5: a source-scanning guard fails CI if any handler reaches `funding_owed` / `worse_of_health_price` / `health_price_with_staleness` outside its allowlisted proven wrapper, so a refactor can't silently bypass the proven funding/health helpers. Surfaced a tracked residual (the `liquidate_position_v2` inlined health gate) to collapse via a devnet-verified PR.
