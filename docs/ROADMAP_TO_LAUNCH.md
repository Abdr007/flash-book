...
| 2.2 | isolated-position ADL redirect (cross has ADL; isolated doesn't) — close asymmetry + prove | eng | **DONE (PR #315) — asymmetry ALREADY CLOSED in code; PR adds the missing proof.** Investigation found `auto_deleverage` already routes isolated positions correctly: eligibility admits `collateral_quote_lots > 0` (isolated) even for a multi-leg trader (`lib.rs:9555`); the health check uses `assess_margin_unified`, which assesses each isolated leg against its OWN bucket (the passed cross-collateral is ignored for it); and the bankruptcy-price uses the isolated bucket (`lib.rs:9639`). The roadmap premise ("isolated doesn't") was stale. Test `auto_deleverage_accepts_isolated_underwater_leg`: the same 2-leg setup that is REJECTED for a cross underwater leg (Custom 8207) is ACCEPTED when the underwater leg is isolated — the exact asymmetry, proven closed. |
| 2.3 | on-chain payout walk for referrer/builder/creator shares (today emit-only) | eng | **DONE (program) — trustless escrow-accrual; devnet cycle pending.** The three shares are no longer emit-only: `FeeAccrualAccount` PDA `seeds=[b"fee_accrual", recipient]` (`recipient` + `accrued_quote_lots` + bump; state.rs) holds each recipient's on-chain balance. In `apply_fill`, a PDA-scan (`find_fee_accrual`, mirroring the ring/outbox scanners — matches by discriminator + recipient, **not** position) discovers an optional accrual account per recipient in `remaining_accounts`; if present, `min(share, available_surplus)` (`available_surplus = net_fee − maker_rebate − insurance_contribution`) is written in place (`accrue_to_fee_account`) and the global `insurance_fund.total_fee_accrued_lots` liability grows — **absent ⇒ exact pre-2.3 emit-only path** (verified: all 117 prior integration tests unchanged-green). Cold `claim_fee_accrual` (recipient-signed) transfers the balance vault→recipient ATA (PDA-signed), zeroes the accrual + decrements the liability, gated by the same Kani-proven solvency floor — now `vault ≥ insurance + flp + total_fee_accrued_lots` (`verify_protocol_solvency` extended). Surplus-cap ⇒ total distributed ≤ `net_fee` collected, so accrual is Residual-neutral by construction. **Machine-proved** (`ResidualConservation.lean`, axiom-clean): the refined identity `V = C + I + FeeAccrued + R'` (`conserved5`), `conserved5_iff_conserved` (2.3 splits a G4 bucket, doesn't weaken it), and the marquee `fee_share_accrue_preserves_solvency` — accruing `a ≤ available_surplus` (the handler's `min` cap) both conserves AND keeps `R' ≥ 0`, so the extended floor `V ≥ C + I + FeeAccrued` never breaks; `fee_share_claim_preserves_solvency` is solvency-neutral. 3 new tests (init / claim-payout / claim-rejects-empty) + IDL regen; 383 lib + 120 integ green, clippy/fmt/SBF 0-warn, Lean clean. **Remaining: devnet acceptance of the full accrual-through-apply_fill→claim cycle** (host-tested via seeded state; the live ER round-trip is the last evidence step). |
...
| Item | Scope | Owner | Status | Evidence |
|---|---|---|---|---|
| 3.1 | per-source-domain realizable credit: cap each profitable leg's usable PnL by `credit_rate = min(1, backing/claims)` of the opposing side of that same market — manipulated/thin/stale market ⇒ credit collapses ⇒ paper profit can't back margin/cure/withdraw (closes the HL-$20M oracle-pump class) | eng | **SPLIT — mark-manipulation vector DONE; ability-to-pay layer = documented remainder.** The ACTUAL HL attack (thin-market mark PUMP) is proven impossible on the real engine: `worse_of_health_price` + staleness gate + the new adversarial Kani `jelly_mark_manipulation_yields_no_usable_equity` (VERIFICATION SUCCESSFUL, non-vacuous) — a mark manipulated in the attacker's favour can NEVER move the health price past the honest live oracle, so a pump converts to ZERO usable equity. The SECOND layer (per-domain ability-to-pay cap) is **now WIRED in the engine** — `MarketAccount.paper_profit_haircut_bps` (= `BPS_DENOM − credit_rate`, `credit_rate = min(1, backing/claims)`) is cranked by the authority/sequencer via `set_paper_profit_haircut` (a keeper computes ability-to-pay off-chain, exactly like the oracle/funding cranks), and `risk::assess_margin` scales every position's usable POSITIVE unrealized PnL by the complement (`risk::haircut_positive_pnl`) at BOTH the equity and stress-scenario sites. Only positive uPnL is haircut (losses count in full) ⇒ it can only RAISE the requirement (conservative-safe); `haircut = 0` default ⇒ byte-identical pre-3.1. **Proven**: Lean wired-form theorems (`usableHaircut_le_pnl`, `full_haircut_zero_credit`, `zero_haircut_identity`) + 5 unit + engine A/B `paper_profit_haircut_flips_health` (a winning position healthy at haircut 0, LIQUIDATABLE at full haircut) + on-chain `set_paper_profit_haircut_cranks_and_gates_auth`. Default-off; 12-byte MarketAccount growth = devnet migration (boxed `AcceptAuthorityTransfer` for SBF stack). **Remaining:** the sophisticated off-chain ability-to-pay CRANKER (backing/claims aggregation) + a devnet cycle — the on-chain primitive + margin gate are done. | mark-manipulation: adversarial Kani on real symbol; ability-to-pay: **wired + Lean-proven + engine A/B**, off-chain cranker remains |
...
| 3.3 | exclusive per-domain close serialization: immutable `close_id` + `max_close_slot` on the liquidation/bankrupt-close path (deadlock/livelock impossible by construction) | eng | **DISPOSITION: the deadlock/livelock is ALREADY PREVENTED in code** — the H3 dedup guard rejects a duplicate `order_type==3` close injection while one rests (`lib.rs:9258` single-leg, `:10192` portfolio → `RateLimited`), so at most ONE close rests per position; and the H-B keeper/authority `retire_liquidation_order_v2` path (PR #300, tested) always clears a stranded close, so it can never permanently block. Together: no livelock (no duplicate stacking), no deadlock (a stranded close is retirable). The formal `close_id`/`max_close_slot` construction is a stronger-by-construction restatement of this guarantee whose marginal value does not justify a MarketAccount **layout migration** + SBF-stack rework right now — deferred as a hardening, not an open gap. | — |
...
| 4.3 | scale/ladder USER order type (FLP quoter ladder exists; expose a user version) | eng | **DONE (PR #318).** New `place_ladder_order` ix: places `num_levels` (≤ `MAX_LADDER_LEVELS`=20) resting limits at `base ± i·step` stepping away from mid (bids down, asks up), each a full `place_limit_v2_core` so every per-order gate (tick/5-sig-fig/min-notional/band/OI/intake-margin) applies per rung; reduce_only disallowed. No new state. Test `place_ladder_order_rests_multiple_levels`. |
| 4.4 | published margin-tier table (leverage-steps-down-with-notional) + enable coded-but-inactive OI-crowding surcharge | eng | **DONE (program)** — the OI-crowding maintenance surcharge is no longer dormant. `MarketParams` now carries `oi_mmr_slope_bps_per_million_lots` + `oi_mmr_max_extra_bps`, and all **13** on-chain `RiskMarketSnap` construction sites thread the real per-side OI (`market.oi_long_lots`/`oi_short_lots`, selected by the position's side via the new `oi_side_lots` helper) into the snapshot the margin engine reads. The surcharge math + its application in `effective_mmr_bps` were already unit- + Kani-proven (`matcher::risk::oi_scaled_mmr_extra_bps`); this wires the config source + call sites. `slope = 0` is the default ⇒ byte-identical to pre-4.4 behaviour until a market opts in via `update_market_params`; because the term is purely additive to maintenance bps, enabling it can only ever *raise* the requirement (never under-margins an existing position). Adds `oi_crowding_wiring_tests` (per-side selector) + the `default_params()` fields; IDL regenerated. Growing `MarketParams` shifts trailing `MarketAccount` offsets = a migration (devnet re-init; consistent with 4.1). 383 lib + 117 integration green, clippy/fmt/SBF/IDL clean. |
| 4.5 | tranched liquidation (positions above a notional threshold liquidate in tranches) | eng | **DONE (program) — devnet cycle pending.** `liquidate_position_v2` already injects a resting close order of `close_size` lots (partial close supported) and already rate-limits re-liquidation via `liquidation_cooldown_slots`; 4.5 adds the tranche size cap that makes large positions unwind over multiple cooldown-spaced calls. New `MarketParams.max_liq_tranche_lots` (state.rs); after `close_size` is resolved, `liquidate_position_v2` clamps it to the tranche (`close_size.min(max_liq_tranche_lots)`) so the injected liquidation order is at most one tranche — market-impact mitigation. `0` = disabled (unbounded single close = existing behaviour); default-off ⇒ byte-identical for existing markets (a devnet-only migration, like 4.1/4.4). **Deliberately NOT applied to `auto_deleverage`** (ADL fires only when insurance is already below its pause threshold — throttling there would worsen the deficit). Test `liquidate_position_v2_caps_close_at_one_tranche` drives a real underwater 3-lot liquidation and asserts the injected close order is 1 lot (via the byte-faithful book decoder), not 3. NB: the 8-byte param growth tipped `place_basket_order_n_v2` over the 4 KB SBF frame → fixed by extracting the per-leg deserialize/validate into an `#[inline(never)] basket_n_load_leg` helper (byte-identical logic; basket security test still green) + boxing that context's `flp_exposure`. 383 lib + 121 integ green, clippy/fmt/SBF 0-warn, IDL regen. **Remaining: devnet acceptance.** |
| 4.6 | user reduce-only flag on the v2 place path | eng | **DONE (PR #309, all CI green).** The v2 place paths (`place_limit_v2_core`, `place_taker_order_v2`) already honored `reduce_only` (resting flag + inflight-aware maker clamp re-caps to reducible size at fill ⇒ never opens/flips); PR #309 honors it on `modify_order_v2` and **PR #311 on `vault_place_order_v3`** (reduce-only exempt from the H-A intake gate; matcher clamps against the vault's own position at fill) — so `reduce_only` is now honored **uniformly across `place_limit` / `place_taker` / `modify` / `vault`**, no residual. Tests `modify_order_v2_accepts_reduce_only_flag` + `vault_place_order_v3_honors_reduce_only_and_exempts_the_intake_gate`; safety covered by `v1_reduce_only_trigger_two_takers_cannot_flip_position`. |
...
- **Program changes**: **ALL BUILT** — the last first-party program-change items
  ...
  **Devnet deploy (2026-07-12):** merged main built + deployed to the throwaway `BRtnEAZ6…` (upgrade sig `5PVsfo9…`); **deployed bytecode sha256 == local artifact** (`cdcfaeaa…`), so devnet runs bit-for-bit main. Live acceptance (public devnet): **instruction surface** (`feature_acceptance_2026_07_12.mjs`, 2/2) — `init_fee_accrual` creates the recipient PDA, `claim_fee_accrual` on an empty accrual reverts ZeroSize (0x1c22) through the full account path. **2.3 value-moving HAPPY PATH now also devnet-proven** (`fee_share_payout_acceptance.mjs`, 2/2, keyed devnet): the full armed matcher flow — init market+book+ring+outbox → maker rests a bid → taker sweeps (pushes the keccak commitment) → `apply_fill` settles WITH the ring AND accrues the referrer share (**900 quote-lots on a real matched fill**) → `claim_fee_accrual` pays out (**referrer ATA = 900, accrual zeroed**), real sigs. **4.5 tranche clamp now ALSO devnet-proven** (`tranche_liquidation_acceptance.mjs`, 1/1, keyed devnet): armed market (`max_liq_tranche_lots=2`) → open a real 3-lot long → `set_envelope_config` + `update_oracle` (first envelope observation seeds+skips the cap) drops the oracle 100000→40000 so the long goes underwater → `liquidate_position_v2` injects a close order of exactly **`size_lots=2`** (asserted via the `LiquidationInjectedV2Event`), not 3, real sig. **4.4 OI-surcharge margin effect now ALSO devnet-proven** (`oi_surcharge_acceptance.mjs`, 3/3, keyed devnet): a clean A/B — the SAME zero-PnL 10-lot long (health = collateral, no adverse move) is **healthy on a `slope=0` market** (`liquidate_position_v2` reverts `NotLiquidatable` 7403) but **liquidatable on an otherwise-identical `slope>0` market** (liquidation injects, per `LiquidationInjectedV2Event`) — so the OI surcharge ALONE flips liquidatability, real sig. **All three money-path features (2.3, 4.4, 4.5) are now live-behavior-proven on devnet; no live-behavioral gap remains.** (**4.8 CLOSED** + devnet-accepted, PR #300; the H-B cancel-lock
