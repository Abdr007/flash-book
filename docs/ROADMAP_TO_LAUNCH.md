...
| Item | Scope | Owner | Status | Evidence |
|---|---|---|---|---|
| 3.1 | per-source-domain realizable credit: cap each profitable leg's usable PnL by `credit_rate = min(1, backing/claims)` of the opposing side of that same market — manipulated/thin/stale market ⇒ credit collapses ⇒ paper profit can't back margin/cure/withdraw (closes the HL-$20M oracle-pump class) | eng | **DESIGNED + core-math VERIFIED** — `docs/PER_DOMAIN_CREDIT.md`; credit-from-rate safety (usable ≤ pnl, zero-rate ⇒ zero credit) Kani-verified locally (SUCCESSFUL, 0/5); durable proof routed to Lean (128-bit, ~300-700s in CBMC — not CI-resident). REMAINDER: Lean theorem + engine-wiring (assess_margin / withdraw / cure) + deploy + live-verify | Lean per-domain conservation + live devnet |
