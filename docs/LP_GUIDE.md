...
Flash Book's FLP pool uses an ERC-4626-style **NAV-based vault**. Multiple LPs
deposit USDC; each receives shares proportional to their share of NAV at
deposit time. As the pool earns (maker rebates, realized PnL from FLP fills,
...
The protocol authority calls `initialize_flp_exposure(initial_capital)`. The
...
Call `deposit_flp_capital_ix` with your USDC ATA + the amount. The chain:
...
### 3. Earning (passive)
Every time the FLP pool wins a fill, your shares appreciate. Three sources:
- **Maker rebates** from `apply_flp_fill` flow into `total_capital`.
- **Realized PnL** from FLP positions closing accumulates in `realized_pnl`.
- **Toxicity tax share** (when VPIN > 0) flows into `total_capital`.

You don't do anything; share value rises automatically.

### 4. Withdraw
Call `withdraw_flp_capital_ix` with the number of shares to burn. The chain:

1. Computes `amount = shares_to_burn × NAV / shares_outstanding`.
2. Validates that the pool can solvently part with that capital. If
   `markets_count > 0` (FLP has open positions), you must pass each active
   ...
   FLP is fully exposed, your withdraw rejects until exposure drops.
...
`programs/flash-book/tests/proptest_new_features.rs`):
...
- **Bootstrap is 1:1.** When the pool is empty (NAV ≤ 0 OR shares = 0), the
  ...
  on bootstrap.
- **Withdraw guard scales with exposure.** Open FLP positions block
...
- **No early redemption guarantee.** If the pool is fully exposed and the
  market is volatile, you may have to wait for FLP to unwind before you can
  withdraw.
- **No per-LP fee tier.** All LPs receive PnL pro-rata by share count.
  ...
  trade as a taker, not when the FLP earns.
...
A: Read `FlpExposureAccount.total_capital_quote_lots` and `.realized_pnl` and
...
**Q: What if the FLP pool goes underwater?**
